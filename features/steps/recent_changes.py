...
def then_report_contains_last_updated_with_format(context, reporting_format):
    ...
    assert search(updated_pattern,
                  context.scanner_report), \
        f'Last Updated date not found in {context.scanner_report} using {reporting_format}'
    assert search(authored_pattern,
                  context.scanner_report), \
        f'Last Authored date not found in {context.scanner_report} using {reporting_format}'
